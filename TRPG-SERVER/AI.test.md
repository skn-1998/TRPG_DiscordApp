...
> 検証（Fable 独立再実行）: build / focused 5 suites・56 tests / 対象 eslint 0 error / madge 586 files・循環ゼロ / 全量 **228 suites / 3169 tests** 緑。レビューは Opus round2 pass・Codex round2 needs-fix（Medium 1）→ round5 修正。経緯と設計判断は AI.refactor.md 第3群-b 節を参照。
>
> **2026-07-28 第3群-a・APP_PIPE 一本化**: production の validation を共有 `APP_VALIDATION_PIPE_PROVIDER` 1本へ統一した。
> round1 で一時導入して撤去したのは `ForbidUnknownUserBodyFieldsGuard` であり、認証を担う既存の `JwtAuthGuard` は変更していない。
>
> `app.module.spec.ts` を新設し、Promise import を await する production module graph 走査によって APP_PIPE provider 一意性、全10 controller 名の厳密一致、継承 method を含む class/method/parameter local pipe 不在、`main.ts` の `useGlobalPipes` 不在を固定した。
> `user-profile.dto.spec.ts` は unknown key の assert を400拒否から whitelist strip へ反転し、`auth.controller.http.spec.ts` は APP_PIPE と controller 明示再構成の二重防御を表す名前へ変更した。
> whitelist の HTTP 単独因果は新設 `character-sheet-http-validation.spec.ts` へ分離し、`non-finite-formula-save.reproduction.spec.ts` を非有限式保存の再現責務へ戻した。
> `test/test-app.module.ts` にも provider はあるが、参照元0かつ `*.e2e-spec.ts` 0本のため、現状はデッドな e2e 準備状態である。
>
> round5b は build、focused 6 suites / 55 tests、全量 226 suites / 3109 tests、madge 583 files / 循環ゼロを確認した。
> **最終（a6）** は build、focused 3 suites / 15 tests、全量 **226 suites / 3110 tests**、madge 583 files / 循環ゼロ。
> a6 で追加した3変異（走査の `await` 削除・method 収集の空配列化・parameter 収集の空配列化）はいずれも狙った spec が赤になった。
>
> **注記**: 以下 2026-07-12 の各段落が名乗る「（未コミット）」は**執筆時点のラベル**であり、
> 改善単位2・5 をはじめ相当数はその後コミット済み（例: `character.repository.ts` の owner-qualified API は HEAD に存在）。
> 未コミットの実装が残っている意味ではない。
>
> **2026-07-12 変更容易性改善単位7・Character実DB統合テスト隔離（未コミット）**: 通常Jestから `character.integration.spec.ts` / `character.crud.spec.ts` を除外し、専用 `jest.integration.config.js` と `test:integration` でDocker CLIの使い捨て `mongo:7` だけへ接続する契約に変更した。通常setupはhost `.env` の外部URIをlocalhostへ上書きし、専用境界はprovider marker、run ID、`127.0.0.1` の動的port、run固有DB名が一致しないURIを拒否する。残留state/lock、接続失敗、cleanup失敗をfallback/skipせず失敗として扱う。隔離境界 **2 suites / 5 tests**、実DB **2 suites / 19 tests**。実DBREDで判明したMongoose `Mixed` のdeep mergeと`undefined`→BSON `null`を、repositoryのpipeline `$literal` 置換と定義済みoptional keyだけの組立てで解消。Fable初回は3観点Passの `Approved with follow-up`。repository直接書込みにもundefined除去・5セクション正準形guardを追加し、残留lock/cleanup失敗を明示化、pipeline後の`updatedAt`を実DBで固定した。追跡・最終確認はいずれも **Approved**、必須指摘0、3観点Pass。属性内の明示的undefinedを拒否し、空AttributeValueとoptional省略の受理も境界testで固定。最終対象 **2 suites / 81 tests**、実DB **19 tests**、残留0。通常Jestは並行変更前に198 suites / 2,567 tests全件成功、最終時は改善単位1のcompile error 1 suiteを除く199 suites / 2,588 tests成功。現在のtypecheck/build停止理由は `backfill-template-pin.spec.ts:36` と `roll-palette.handler.ts:43` の対象外2型エラーである。
>
> **2026-07-12 変更容易性改善単位6・AttributeValue正準形（未コミット）**: `AttributeValue` の実行時判定を `core/types` に追加し、許可6キー、有限数の`values`、文字列`dice`、プレーンオブジェクトという事前条件をHTTP DTO・イベント・作成コア・CharacterServiceで共用した。辞書に対する誤った`@Type`変換、`dice`欠落、プリミティブから空属性への暗黙変換、Discord編集の`null`保存を解消。初回REDは7 assertion failures + 2 compile errors、focused **5 suites / 114 tests**、拡張 **3 suites / 60 tests**。Fable初回レビュー `Changes requested` のlegacy read-merge-write回帰を、repository読出専用adapterで修正。部分更新guard、有限数入力、domain error非リトライも追加RED後に修正し、追跡5 suites / 115 tests成功。`typecheck:test`、build、対象lint 0 errors。改善単位7の隔離MongoDBで新規/legacy往復 **2 suites / 19 tests** も成功した。
>
> **2026-07-12 変更容易性改善単位3・未処理Promise rejectionの可視化（未コミット）**: `test/utils/jest-setup.ts` がJest Circusの `unhandledRejection` listenerを `process.removeAllListeners` で消し、空handlerへ置換していた6行を削除。これによりJest既定の「未処理rejectionをtest failureとして報告する」契約へ復帰した。リモートDB接続を持つ `character.integration.spec.ts` / `character.crud.spec.ts` の2本だけを除いた同一集合で、変更前 **186 suites / 2,433 tests成功**、変更後も **186 / 2,433成功**。追加で露出した未処理rejectionは0件。`typecheck:test`、build成功、check:circular 493 files・循環0。process-levelの抑制は検索上0件。Claude CLIレビューは **Approved**。残存確認は改善単位7でリモートDBを隔離後、除外2 suiteを含めて実行する。今後suite単位の非決定的失敗が出た場合は、握りつぶされていた未待機Promiseを第一候補として調査する。
>
> **2026-07-12 変更容易性改善単位4・Discord REST操作契約（未コミット）**: facadeが返すSDK `Message` / `Channel` とcontrollerが期待するHTTP結果の不一致を、`success` 判別unionへ統一。成功ID必須、失敗error必須、SDKオブジェクト非公開を不変条件とした。操作optionsも `any` から共有interfaceへ変更。単数/複数Embedの統合、複数 `embeds` だけの送信、`#RRGGBB` 色の数値化と境界、文字列ChannelTypeのSDK境界変換、未知文字列・数値thread型のSDK前拒否を契約テストで固定した。REDを確認後、**4 suites / 88 tests成功**、`typecheck:test`、build成功、check:circular 496 files・循環0、lint 0 errors（既存86 warnings）を確認。Fable初回・追跡レビューはいずれも重大指摘なしの `Approved with follow-up` で、適用した指摘は追加specまで完了。
>
> **2026-07-12 変更容易性改善単位5・User / Character認可契約（未コミット）**: 変更前6 suites / 102 testsを固定。Character repositoryのowner-qualified API不存在3件をRED確認し、個別取得・更新・削除を `characterId + discordUserId` の単一queryへ変更。Userはcontroller全体のJWT guard、本人path制約、HTTP専用profile DTO、token-free presenterを追加し、主体ID/mass assignment/token漏えい/他人pathの6件をRED確認。DTOのstrict ValidationPipeとguard metadataも固定した。Fable初回レビューの横断指摘で `/discord/post-character` のID単独経路を追加RED後に同じowner契約へ修正し、複合Param DTOも実class化。最終 **9 suites / 157 tests成功**、`typecheck:test` 成功。対象不在と非所有者は同じ404、OAuth token更新は内部AuthService経路だけを許可する。Fable追跡レビューは **Approved**。
>
> **2026-07-12 変更容易性改善単位2・検証処理の修復（未コミット）**: `package.json` に読取専用 `lint:check`、明示修正用 `lint:fix`、`typecheck:test` を追加し、既定 `lint` は `lint:check` へ委譲。`typecheck:test` は `tsconfig.spec.json` と `--incremental false` を使用する。Madge 3コマンドは `--ts-config tsconfig.json` へ統一し、Jestの個別coverage閾値を移設済み `src/core/http/error-handler.ts` へ追従。Jest/tscのalias差を防ぐため `tsconfig.spec.json` に `@*` aliasesを追加。新ゲートで露出した23件は、参照ゼロの旧 `test/factories/test-data.factory.ts` と `test/utils/test-helpers.ts` を削除し、test auth 2ファイルの `CookieService` importを現 `core/http` へ更新して解消。Lint唯一のerrorはlistener specのstring/enum比較を文字列同士へ揃えて解消。**検証**: `typecheck:test` 0 errors / `lint:check` 0 errors・86 warnings / listener spec 1 suite・4 tests成功 / build成功 / check:circular 493 files・3 warnings・循環0 / Jest `--showConfig` で旧coverage pathなし・現pathあり。通常のpnpm実行は既存workspace差分により `ERR_PNPM_VERIFY_DEPS_BEFORE_RUN: Setting overrides of lockfile ... is outdated` で停止するため、検証時のみ `--config.verify-deps-before-run=false` を使用しlockfileは未変更。全Jest/coverageはリモートDB分離前のため未実行。Claude追跡監査は重大指摘なし・`Approved with follow-up`。残件は現在未使用のbare `core/*` / `features/*` 等と、既存の死んだ `@auth/*` / `@middleware/*` aliasを3設定で整理する予防的Lowであり、別の設定整理単位とする。次は改善単位3（`unhandledRejection` 可視化）。
>
> **2026-07-07 C 系列完遂に伴う spec 変更＋★フレーク教訓の訂正（C-4〜C-10）**: ①C-4 — 既存 handler spec **無変更**のまま緑（挙動保存の証明）＋parent-channel.util.spec 新設（8）。②C-5 — console spy 2 件を Logger.prototype spy へ（Assert 意図不変）。③C-7 — controller spec の mock を facade へ差し替え（HTTP 応答 Assert 全不変）＋旧ラッパー挙動の characterization 2 件追加・委譲検証のみの discord.service.spec は本体ごと削除。④C-3b′ — monitoring spec を emit 検証から状態/ログベースへ置換・live 配線回帰テストを alert-manager.service.onevent.spec として新設。⑤C-10 — **変更ゼロ**（--detectOpenHandles 全数採取で open handle 0・警告は E-5 で解消済みと確定・180/2401×3 連続クリーン）。**★旧教訓の訂正**: 「suites failed / failed tests 0」フレークは worker リークと**無関係**で、正体は**並行編集中の壊れ spec（現物: character-sheet-template.controller.spec の TS2345＋JwtTokenService 未提供 DI エラー）**。同 spec 修正までの全 suite 検証は `--testPathIgnorePatterns="character-sheet-template"` でベースライン **180 suites / 2401 tests** に固定する。
>
> **2026-07-07 E-6 全体に伴う spec 変更（`04e0b5b`〜`199b5e6`）**: ①E-6a — creation.completed に characterization 2 件追加（discordChannelId あり→thread.create emit／threadId のみ→emit しない=bugfix の固定・RED→GREEN 経由）・thread-creation の「update 1 回・discordThreadId のみ」を times(1) で固定。②E-6e — 新規 `dice-execution.service.spec`（11）＋`dice-save-key.util.spec`（11・2026-06-11 のスレッド親意味論と modal 4 段 fallback の全分岐）＋旧 wrapper spec 7 件を bcdice.util.spec へ移設。jest.mock パスは `src/domains/dice-roll/services/bcdice.util` へ全数追従。③E-6c — E-2a 回帰ガードの否定 assert を「TypedEventService 非注入＝DI レベル保証」へ置換（コメント明記）。④E-6d — repository plain 化（create=save→toObject / update・find 系=lean）を repository spec の新契約で固定・toObject ガードの旧 characterization を「plain をそのまま渡す」へ更新。全 **180 suites 2429 tests 緑**。**申し送り**: 旧 DB レコードの default 欠損は lean 経路で補完されない（character.entity.ts docstring 参照）— 将来 default 依存のテストを書くときは注意。
>
> **2026-07-07 E-2f に伴う spec 変更（`048eab5`）**: ①新規 `character-creation-core.service.spec.ts`（22 tests）— 移設元 handler spec の意味論を踏襲し重複チェック/coc・dnd5e・sw2.5 パラメータ検証/採番/委譲を固定。②`character.creation.requested.spec.ts` — creationCore mock へ差し替え・completed/failed emit Assert 不変。③`character-embed-manager.service.spec.ts` — createCharacter を DI 追従・**completed 自己発行の characterization**＋回帰ガード。④`character-modal-handler.service.spec.ts` — 「成功時は editReply のみ・チャンネルへ二重投稿しない」を characterization（Codex Medium 対応の固定）。全 **186 suites 2555 tests 緑**（+1 suite/+17）。**★教訓**: 全 suite 実行で 1 回「3 suites failed / failed tests 0」の suite 未実行型フレークを観測（直後2連続全緑・C-10 の worker teardown リーク疑い）。**suite failed かつ test failed 0 のときは即再実行で切り分ける**こと。
>
> **2026-07-07 E-2b/E-2c/E-2d に伴う spec 変更（`214db1d`/`d4e5f0b`/`8e51698`）**: E-2a と同標準（characterization-first・Assert 不変・旧 RPC 未使用の回帰ガード）で 4 spec を DI 追従。①dice-roll-character-provider — 直 new から TestingModule 化（回帰ガードを DI 解決で live 化）・null/reject スキップ契約 +2。②character-section-editor — findOne mock 化・reject→null と toObject 互換 +2。③enhanced-character-edit — 該当3テストの Arrange のみ差し替え。④character-modal-handler — **update 成功時に `character.update.completed` を emit する通知連鎖の characterization を追加**（UI 連鎖の生存保証・失敗/null 時は未発行を固定・null 返却ケース +1）。全 **185 suites 2538 tests 緑**（+5＝追加分と整合）。
>
> **2026-07-07 E-2a/E-2e′ に伴う spec 変更（`9e92fb6`/`50926be`）**: ①`character-display.service.spec.ts` — createCharacterEmbed 系を characterization-first で DI 追従（Arrange を waitForEvent mock → `CharacterService.findByChannelId` mock へ・**Assert 不変**＝挙動保証。「タイムアウト→null」前提のケースは「findByChannelId が null」へ意図的更新）。**「旧 RPC（emit+waitForEvent）へ戻っていない」回帰ガード assert を追加**（Codex レビュー Low 指摘反映・E-2 系 slice では今後もこの形の回帰ガードを推奨）。②`channel-name-sync.service.spec.ts` は dead サービス本体ごと削除。全 **185 suites 2533 tests 緑**（186/2541 から −1 suite/−8 tests＝削除分と整合）。
>
> **2026-07-07 C-2 dead-code 撤去に伴う spec 削除（コミット `c8aa131`）**: `dice-parser.service.spec.ts` をサービス本体ごと削除、`dice-orchestrator.service.spec.ts` から削除 8 メソッドの describe 6 個（parserService mock 除去・constructor 1 引数化）、`dice-calculation.service.spec.ts` から `parseAndCalculate` describe＋`evaluateFormula 挙動固定` characterization（`evalRaw` が削除対象 `parseAndCalculate` 経由でしか動かない構造のため併せて削除。private `evaluateFormula` の live 経路は `calculateAndRoll` の STR 代入テストと `arithmetic-evaluator.util.spec.ts` の characterization で引き続きカバー）。**live メソッドのテストは無変更＝挙動不変の証明**。全 **186 suites 2541 tests 緑**（187/2613 から −1 suite/−72 tests＝削除分と整合・新規破損ゼロ）。**★申し送り（未修正バグ）**: レビューで `dice-calculation.service.ts:63` の **await 漏れ**を検出（`diceResult` が Promise のまま→ custom-dice-modal.service.ts:84 の `.rands.reduce` で TypeError＝param-dice-modal 経路の本番バグ）。spec が `mockedDice.mockReturnValue`（本来 **mockResolvedValue**）のため緑のまま見逃されていた。**修正時は await 追加＋spec の mockResolvedValue 化＋Promise 混入を検出する回帰テストを追加**すること。詳細は `AI.refactor.md` 2026-07-07 C-2 節。
>
> **2026-06-11 スレッド内ダイスロールの保存キーを実親チャンネルへ変更＋回帰テスト追加（★未コミット）**: キャラ登録チャンネル外で作ったスレッド内のロールが `/dice-result` に出ない問題の修正（真因・設計は `src/discord/AI.discord.md` 2026-06-11 メモ）。`DiceRollLogicService.resolveSaveChannelId` 新設＝**DB 保存のみ**スレッド内は `channel.parentId`、キャラ解決キー（customId 由来）は不変。`dice-roll-logic.service.spec.ts` に **+4 ケース**（handleDiceRoll/handleSkillRoll/handleCustomDiceRoll の thread→parent 保存、parentId 欠落時の lookup キーフォールバック、イベント payload は lookup キーのままを固定）、`custom-dice-modal.service.spec.ts` に **+1 ケース**（実親がキャラ登録チャンネルより優先）。**既存 spec の期待値は不変**（スレッド外の保存挙動は変えていないため。既定モック channel は GuildText＝非スレッド）。**検証**: build 0 / check:circular No circular / 全 **187 suites 2613 tests 緑**（直前 187/2608 ＋5件と整合＝新規破損ゼロ）。
>
> **2026-06-10 CustomDiceModalService 不具合修正＋回帰テスト追加（★未コミット）**: カスタムダイスモーダルの確認済み不具合2件（①ロール結果が dice-roll 履歴へ一切保存されない、②customId 第2要素を characterId 扱いして live 送出元の channelId と不一致→キャラ名が常に「プレイヤー」）を修正（詳細は `src/discord/AI.discord.md` 2026-06-10 メモ）。`custom-dice-modal.service.spec.ts` へ **+5 ケース**追加: channelId 由来のキャラ解決＋履歴保存 DTO 固定／旧 param 系（characterId）の findOne フォールバック＋`character.discordChannelId` で保存／bare customId 時の `interaction.channelId` フォールバック保存／ロール失敗時の非保存／**保存失敗（createText reject）でも reply は行う**（保存は UX を妨げない）。constructor が 3 引数化（`DiceRollService` 追加）したが、他 spec（`dice-roll-modal.handler.spec` / `handlers.integration.spec`）は plain object mock 注入のため変更不要。**検証**: 対象 spec 14 緑 / build 0 / check:circular No circular(480) / **全 187 suites 2608 tests 緑**（直前ベースライン 187/2603 ＋追加5件と整合＝新規破損ゼロ）。
> dead-code 撤去（詳細は `AI.refactor.md` 2026-06-10 節）に伴い、**dead 挙動のみを固定していた spec を削除**: ①`thread-interaction.service.spec.ts` の `postActionButtons` describe（2件・撤去メソッドの生成のみ固定）、②`handlers.integration.spec.ts` の「character*edit*/dice*roll*/character*info* は dead path かつ未routing」characterization（生成元消滅により存在意義喪失・**handler 登録数 23 の期待値は不変**）、③`dice-orchestrator.service.spec.ts` の preset 委譲 describe 4種（presetService mock 除去・constructor 引数 3→2・`getServiceStats` 期待値を `['DiceCalculationService','DiceParserService']` へ更新）、④`dice-preset.service.spec.ts` をサービス本体ごと削除。live 経路（custom-dice-modal の6メソッド等）のテストは不変。**検証**: 全スイート **187 suites 2603 tests 緑**（ベースライン 188/2625 から削除分 −1 suite/−22 tests と整合＝新規破損ゼロ）、build 0、check:circular No circular(480)、start:dev で 23 handler 不変・DI エラーなし。

> **2026-06-07 develop マージ前ブロッカー＝controller spec 3本の全件未実行を発見・修正（コミット `4d41d94`）**: ブランチ `refactor/ref-path-deadcode-cleanup` を develop へマージする前の Phase 6 全スイート実行で、`auth`/`user`/`character` の `*.controller.spec.ts` 3本が **コンパイルエラーで一度も実行されないまま「緑」と誤記録**されていたことが判明。根因は P1-C(`98d5055`) で `HttpExceptionFilter`/`CharacterHttpExceptionFilter` が `AppConfigService` をコンストラクタ注入するよう変更されたのに、(a) `filterError` ヘルパの手動 `new` が引数不足、(b) `user`/`character` の `Test.createTestingModule` が `AppConfigService` を providers 未提供（コントローラの `@UseFilters(class)` 経由で filter を DI 解決するため、本番は `@Global` AppConfigModule で解決するがテストでは落ちる）。**コンパイルエラーが DI 解決失敗を覆い隠し**、per-slice の対象 spec だけ実行する検証では緑に見えていた（全スイート実行で初めて 3 failed / 41 tests 露出）。**修正**: 正しく直っていた `auth` の TestingModule（`appConfigServiceMock` 登録）を手本に、`user`/`character` へ `AppConfigService` mock を providers 登録＋ describe スコープへ集約し `filterError` と共用、3本とも引数補完。**挙動保存**（`app.environment='test'`→`includeStack=false`＝develop の `NODE_ENV='test'` 時と同一・stack 非含有）。**検証（司令塔）**: build 0 / check:circular No circular(482) / 全 **188 suites 2625 tests 緑**（修正前は 185 suites・3 failed・2564 tests＝壊れた 61 tests 未計上）。**教訓**: per-slice の対象 spec 実行が緑でも、コンパイル/DI エラーは suite ごと未実行になり「緑」を誤認させる。**挙動に影響する変更（constructor signature 等）の後・特にマージ前は、対象 spec だけでなく全スイートを実行**すること。詳細は `AI.refactor.md` 2026-06-07 節。
