...
> ⚠️ **解消状況の追記（2026-07-27・俯瞰#3 時点）**: 本書はレビュー時点のスナップショット。修正の進捗正本は
> `AI.refactor.md`。以下の Must は修正キャンペーンで**解消済み**（判定列が CONFIRMED のままでも現行 HEAD には不存在）:
> TI-1 リポジトリ側（S0）/ DM-1（S1、当該エンドポイント自体も S8 c2a110c で削除）/ SP-1・TI-2（S2）/ EV-3（S3）/
> DM-2・DC-6（S4）/ DC-1・DC-3（S5）/ DM-3・DM-4（S6 前倒し）/ DC-5・DC-2（S7 1bd75e0）/ M1（S7-a 86e1f15）/
> CE-3（4c582e9）。残 Must: TI-1 の Atlas ローテーション（ユーザー側）/ CE-1 / SP-2 / SP-3。

> ✅ **キャンペーン終了注記（2026-08-05）**: §4 の着手順のうち **mainline（第1〜第4群）＋第5群を消化し、
> 本書を起点とする修正キャンペーンを終了**。経過・検収の正本は `AI.refactor.md`（第○群 各節）。
> 第5群は 5 コミット（`c3b4d1b`〜`5a0e067`）・純減 ≈6,130 行で完了。クローズ時の記録事項:
>
> - **CE-18 は台帳取り込み漏れ**: ID が §4 の第5群行（:209）にのみ出現し、per-observation の詳細
>   （file:line・失敗シナリオ）が本書のどの表にも存在しない。2026-08-05 の fresh 測定でも対象を
>   特定できず**着手不能のままクローズ**（教訓: 台帳へ ID を載せる際は詳細記述の存在を機械確認する）
> - **系統11（analyze-large-files の改行 44 件）は数値再現不能**: 現物実測で warning 94 件中
>   raw newline 0。当該スクリプトは untracked の並行セッション生成物のため不触。
>   台帳の「44件」は根拠として再利用不可
> - 台帳数値の訂正 3 件（サブクラス未到達は 6 種すべて・CL-1 は 14 中 13 死蔵・OV6-3 の
>   TestAppModule は消費者 0）は AI.refactor.md 第5群 #91 節に記録
> - 未消化の low 案件は Task 起票済み（#94/#96/#92/#95 ほか）— 処理義務なし・随時
