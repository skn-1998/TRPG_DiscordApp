...
**作成日:** 2026-07-06
**ステータス:** C-1 完了（2026-07-07・`c27c224`）／C-2 完了（2026-07-07・`c8aa131`・計画 7→8 メソッド＋DiceParserService 丸ごと削除に拡大。検出した await 漏れ latent bug は `61ef9d4` で修正済み）／**C-8 実施済み（案A・ブランチ `refactor/nest-v11-upgrade`・`97dbbef`・全ゲート緑＋Codex「マージ可」。マージはユーザーの手動 smoke 後＝チェックリストは AI.refactor.md 2026-07-07 節）**／**C-3 完了（2026-07-07・`9d523a4`。監視系の広範 dead 配線は C-3b′ として別 slice 化）**／**C-6 完了（同日・`987203d`・実測 122 箇所＝計画の 4〜5 箇所は過少）**／C-4/C-5/C-7/C-9/C-10 未着手

> **⚠ 優先順位の割り込みルール（2026-07-07 Codex 全体バランスレビューにより追加）**: C 系列（掃除）と E 系列は独立だが、
> **実挙動バグを含む E-2（イベント RPC の DI 化・特に E-2a/E-2e）は C-3 以降の掃除より優先する**。
> 「衝突しない」ことと「優先しなくてよい」ことは別。推奨順序: C-8 マージ → E-2a/E-2e → E-2 残り → E-3 → E-4
> （並行余地は C-3/C-6 のみ。C-7 は E-5 後に縮小版、C-4/C-5 は E-2/E-4 後に再評価）。
> また DI・constructor・依存バージョン・event bus を触る slice は対象 spec だけでなく**全 suite 実行を必須**とする（AI.test.md 2026-06-07 の教訓）。
