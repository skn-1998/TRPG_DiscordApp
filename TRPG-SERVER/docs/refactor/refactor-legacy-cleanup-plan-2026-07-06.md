...
**作成日:** 2026-07-06
**ステータス:** C-1 完了（2026-07-07・`c27c224`）／C-2 完了（2026-07-07・`c8aa131`・計画 7→8 メソッド＋DiceParserService 丸ごと削除に拡大。検出した await 漏れ latent bug は `61ef9d4` で修正済み）／**C-8 実施済み（案A・ブランチ `refactor/nest-v11-upgrade`・`97dbbef`・全ゲート緑＋Codex「マージ可」。マージはユーザーの手動 smoke 後＝チェックリストは AI.refactor.md 2026-07-07 節）**／C-3 以降 未着手
