...
**作成日:** 2026-07-06
**ステータス:** **E-5 完了（2026-07-07・`491ddac`・3層→2層直結・dedup/5分 setTimeout 撤去＝C-10 リーク源の一つと実測確認）→ E 系列の残りは E-6 のみ→**別計画書化完了（2026-07-07・`refactor-entity-boundary-plan-2026-07-07.md`・Codex 設計レビュー済み・ユーザー承認待ち）**／**E-4 全完了（2026-07-07）** — E-4a `5b7d0de`（契約を live 11 種へ一本化・EventName 厳密型化・レガシー契約4ファイル削除・Codex Medium 2件反映=registry membership 検証/契約外 updatedFields dead 分岐撤去）／E-4b `f8454f3`（イベント名リテラル 29 箇所を EVENT_NAMES 化）／E-4c `caaddbe`（**バス1インスタンス化**・空クラス TypedEventEmitter 撤去・isolation spec を1バス固定へ反転）。**E 系列の残りは E-5（3層ルーティング1本化・独立）と E-6（中期・別計画書）のみ**。C-3b への追加材料: metrics-collector の @OnEvent 5 本は emit 元ゼロ・discord-monitor の raw emit（rate-limit/memory/performance.alert）は購読者ゼロ＝監視系の dead 配線。／**E-3 全完了（2026-07-07）** — E-3a `4b2e51d`（update.requested/findBy\* 4 handler＋request ヘルパ 7 本撤去・Registry は creation 1 handler 構成へ）／E-3b `dcb8f0e`（diceroll dead emit）／E-3c `4592d07`（CharacterEditCreationHandler ごと撤去ほか）／E-3d `f62eff5`（discord UI 系 dead emit 13+1 箇所・display 系のゴースト化を記録＝解体は E-5/E-6）／E-3e `89b0a8a`（interactions の素バス注入撤去＝E-4c の前提完了）。**次は E-4**（E-4a 契約一本化＝dead contract 型の整理含む／E-4b EVENT_NAMES 完備／E-4c バス1インスタンス化＋空クラス化した TypedEventEmitter の撤去判断）。／**E-2 全完了（2026-07-07）** — E-2a `9e92fb6`／E-2e′ `50926be`（DI 化から dead 削除に訂正）／E-2b `214db1d`／E-2c `d4e5f0b`／E-2d `8e51698`／**E-2f `048eab5`**（CharacterCreationCoreService を domains へ抽出・embed-manager の壊れ順 RPC＝**診断未計上の3件目の実バグ**も解消・modal-handler の二重 Embed 投稿を Codex Medium 指摘で予防）。E-1 は包含消化。**production の waitForEvent 利用はゼロになった（E-2 の完了条件達成）**。次: E-3（dead contracts/emit 一括掃除。E-2 完了により findBy\*.completed/failed・update/creation の一部が dead 化）→ E-4。E-3/E-4a への参考: `character.update.failed` 恒常購読者ゼロ／`CharacterUpdateCompletedEvent` の契約型ずれ（既存）。
