...
import { DicePageFirstButtonService } from './dice-page-first-button.adapter'

// DiceRollPaginationService へ「先頭ページ計算・状態取得・コントロール生成」を委譲するオーケストレーター。
...
describe('DicePageFirstButtonService', () => {
  let pagination: PaginationMock
  let service: DicePageFirstButtonService
  ...
  beforeEach(() => {
    ...
    service = new DicePageFirstButtonService(pagination as unknown as DiceRollPaginationService)
  })

  it('customId から channelId/messageId を取り出し、updatePage に first 方向を渡す', async () => {
    // Arrange
    pagination.updatePage.mockReturnValue(dummyPage)
    pagination.getPaginationState.mockReturnValue({ totalPages: 3 })
    const interaction = createMockButtonInteraction({ customId: 'dice-first*msg1*ch1' })
    ...
    expect(pagination.updatePage).toHaveBeenCalledWith('ch1', 'msg1', 'first')
  })
  ...
  it('customId に messageId/channelId が欠けるとき followUp で警告し委譲しない', async () => {
    // Arrange
    const interaction = createMockButtonInteraction({ customId: 'dice-first' })
  ...
  it('既に最初のページ(updatePage が null)のとき followUp で通知し editReply しない', async () => {
    // Arrange
    ...
    // Assert
    expect(interaction.followUp).toHaveBeenCalledWith(
      expect.objectContaining({ content: expect.stringContaining('既に最初のページ'), ephemeral: true })
    )
    expect(interaction.editReply).not.toHaveBeenCalled()
  })
