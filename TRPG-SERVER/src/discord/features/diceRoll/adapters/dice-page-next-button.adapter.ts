...
export class DicePageNextButtonService implements discordButtonType {
  public data = new ButtonBuilder()
    .setCustomId(DicePageCustomId.template('next'))
    .setLabel('>')
  ...
  async execute(interaction: ButtonInteraction<CacheType>): Promise<void> {
    try {
      ...
      const newPage = this.paginationService.updatePage(parsed.channelId, parsed.messageId, 'next')
      if (!newPage) {
        await interaction.followUp({
          content: '最後のページです。これ以上先のページはありません。',
          flags: MessageFlags.Ephemeral
        })
        return
      }
