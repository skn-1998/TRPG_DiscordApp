...
export class DicePagePrevButtonService implements discordButtonType {
  public data = new ButtonBuilder()
    .setCustomId(DicePageCustomId.template('prev'))
    .setLabel('<')
  ...
  async execute(interaction: ButtonInteraction<CacheType>): Promise<void> {
    try {
      ...
      const newPage = this.paginationService.updatePage(parsed.channelId, parsed.messageId, 'prev')
      if (!newPage) {
        await interaction.followUp({ content: '最初のページです。これ以上前のページはありません。', ephemeral: true })
        return
      }
