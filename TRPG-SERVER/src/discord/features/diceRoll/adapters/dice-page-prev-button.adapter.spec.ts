...
describe('DicePagePrevButtonService', () => {
  let pagination: PaginationMock
  let service: DicePagePrevButtonService
  ...
  beforeEach(() => {
    ...
    service = new DicePagePrevButtonService(pagination as unknown as DiceRollPaginationService)
  })

  it('customId から channelId/messageId を取り出し、updatePage に prev 方向を渡す', async () => {
    // Arrange
    pagination.updatePage.mockReturnValue(dummyPage)
    pagination.getPaginationState.mockReturnValue({ totalPages: 3 })
    const interaction = createMockButtonInteraction({ customId: 'dice-prev*msg1*ch1' })
    ...
    expect(pagination.updatePage).toHaveBeenCalledWith('ch1', 'msg1', 'prev')
  })
  ...
  it('customId に messageId/channelId が欠けるとき followUp で警告し委譲しない', async () => {
    // Arrange
    const interaction = createMockButtonInteraction({ customId: 'dice-prev' })
