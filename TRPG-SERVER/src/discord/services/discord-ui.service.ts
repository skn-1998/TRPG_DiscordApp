...
export class DiscordUIService {
   ...
   * キャラクター作成完了通知の送信
   */
  async sendCharacterCreationNotification(channelId: string, character: Character, userId?: string): Promise<void> {
    const message = userId
      ? `<@${userId}> キャラクター「${character.characterName}」が作成されました！🎉`
      : `キャラクター「${character.characterName}」が作成されました！🎉`
   ...
   * ウェルカムメッセージの送信
   */
  async sendWelcomeMessage(channelId: string, userId: string, characterName: string): Promise<void> {
    const message =
      `<@${userId}> ようこそ！\n` +
      `このチャンネルは「${characterName}」専用のチャンネルです。\n` +
      `キャラクター情報の編集や、ダイスロールなどが行えます。\n\n` +
      `コマンドを使用してお楽しみください！ 🎲`
