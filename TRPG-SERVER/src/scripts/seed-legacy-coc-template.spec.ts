 ...
 * 「publish 検証に落ちたら挿入しない」「v4 が published になった後でだけ旧行を deprecate する」
 * を守ることを検証する。
 *
...
describe('seed-legacy-coc-template', () => {
  ...
  it('dry-run は挿入せず insert 判定だけを返す', async () => {
    ...
    expect(harness.findById).toHaveBeenCalledWith('legacy-coc-v4')
  ...
  it('execute は seed 実体の値をそのまま published 行として 1 件挿入する', async () => {
    ...
    expect(harness.create).toHaveBeenCalledWith({
      // 旧 `legacy-coc-v3` 行を残したまま出し直すため、seeder が挿入する id は v4 側だけになる
  ...
  it('execute は v4 を挿入した後に旧 legacy-coc-v3 を deprecate する', async () => {
    const harness = createHarness()
    // 戻り値は「published 行が 1 件一致した」ことだけを表す。中身は deprecatedPrevious に影響しない。
    harness.deprecatePublished.mockResolvedValueOnce(existingRow({ templateId: 'legacy-coc-v3', status: 'deprecated' }))
    ...
    expect(harness.deprecatePublished).toHaveBeenCalledWith('legacy-coc-v3', 'system')
    // 順序そのものを固定する。v4 が published になる前に旧行を落とすと選べる行が消える。
  ...
  it('v4 の insert に失敗したら旧行を deprecate しない', async () => {
    const harness = createHarness()
    harness.create.mockRejectedValueOnce(new Error('write failed'))
  ...
  it('dry-run は deprecate を実行せず予定だけを記録する', async () => {
    ...
    expect(harness.deprecatePublished).not.toHaveBeenCalled()
    expect(harness.logger.log).toHaveBeenCalledWith('would deprecate templateId=legacy-coc-v3 author=system')
  ...
  it('deprecate が例外を投げたら exit code 1 を返す', async () => {
    ...
    // v4 の挿入自体は成功しているので inserted は下げない
