...
| 項目 | 状態 |
|---|---|
| **TR レーン（L-2 (c)＋track 昇格 TR-1〜TR-6）** | **実装完了（2026-08-14・大粒度 #20/#21 とも blocking 0）**。engine 契約 A・全経路 advisory（raw canonical）・server 有限性検査 1 本化・front 表示（15 / 10 超過明示）・エディタ track 作成/編集まで。正本 = track-roll-on-create-promotion-draft.md（冒頭に実装コミット 13 本）・証跡 = review-results/impl-u14/tr-lane-acceptance.md。**TR-D1 = (a) 裁定・TD1 e7ee7103 で消化済み。checkboxes 視覚 = TD3 69232eef・未入力 0 表示統一 = TD2 3e91c3ac（いずれも 2026-08-14・大粒度 #22 blocking 0）**。**〔レーン全消化 2026-08-14〕試しロール = TD4 8e9ebf3f・CL-6 出目提示 = TD5 cbb27e3a・role 編集 = TD6 9e2188bb・大粒度 #23 消化（blocking = preview total の評価値統一。突合 = review-results/impl-u14/big23-integration.md）。残 = ユーザー裁定枠のみ（engine の resource role 到達可能性検査ほか — 正本 draft §残スライス）** |
...
| **L-2 裁定** | rollOnCreate 修正方針（a 型昇格／b seed 付与／c 常時ロール — 裁定資料は c 推奨）**【裁定 2026-08-13・ユーザー】(c) 常時ロールを採用**。**付帯裁定 = track の作成時ロールは廃止せず正式契約へ昇格する**（正本 = track-roll-on-create-promotion-draft.md）。**昇格の主要裁定済み（2026-08-13・ユーザー）= 契約形 A（TrackField.rollOnCreate 内包・出目 canonical・二形態廃止）＋範囲意味論は全経路 advisory 化（提出・± も超過許可・数値 15/10 明示・± raw 基準・gauge 塗りのみ cap。既存の提出検査・悪化拒否・raw/投影 clamp・422 の pin 群を反転）**。実装順序 = 未定・着手はユーザーの再開指示待ち。**〔進捗 2026-08-13〕TR-1（736e5305）・TR-2 engine 契約 A（16f91cc）・D-R4（d2d0759）・L-13（51d83bf）済み。大粒度 #18 = 二重レビュー一致でクリーン・統合フェーズ不要（突合 = review-results/impl-u14/big18-integration.md）。順序制約 = TR-4b 完了前に TR-5 を始めない（raw/capped 二語彙の front 流入防止）。追加の司令塔裁定 2 件（itemFields 内 track の publish 拒否・提出値衝突 422）は正本 draft §裁定 4/5。CL-6 = TD5 cbb27e3a で消化済み（2026-08-14。作成応答の wire required 配列＋front Modal 表示）。**〔完了 2026-08-14〕TR-1〜TR-6 実装完了（§0 参照・大粒度 #20/#21 blocking 0・残 = TR-D1 凍結＋残スライス）** | 再現 spec の赤解消・D-3 の作成時ロール受入・**SM-F（SM-1 proof 原子性）の着手**（2026-08-12 実測: proof/nonce は契約・production とも完全未実装〔dice preview の wire は total/details のみ〕。proof の消費者 = ウィザードの roll step UI も未構築。作成時ロールの在り方が本裁定で決まる前に protocol を先行実装すると死んだ抽象になるため、キュー #11 の SM-F はブロック扱い） |
| **L-13** | legacy-coc テンプレートの DB 投入方法（手動 publish or seeder 新設・所有者/version）**【裁定 2026-08-13・ユーザー】seeder 新設を採用**。**〔実装 2026-08-13〕** seeder = src/scripts/seed-legacy-coc-template.ts（backfill 家風・dry-run 既定・冪等キー templateId＋version・seed 実体無改変・publish 3 検証再利用〔visibility=public 要求のみ規約例外として除外 — seed は private が正で書き換え禁止のため。code 内 NOTE あり〕）。**〔2-2 型不変条件・#18 CL-2 → 大粒度 #24 DLZ で縮小 2026-08-15〕** publish 検証 3 段は共有 collector（validation/template-publish-validation-issue.collector.ts）1 本に集約され、domain validator と seeder は入口規約（visibility 要求・失敗集約形）だけを持つ。段の脱落は両入口の spec が赤にする（変異 MT1/MT2 で機械確認）。**残る手組み面は front エディタ runValidation のみ** — domain 側の検証増減時は front の追随を指示書に明記すること（front には投影キー検査が元々無い = 既知乖離のまま） | D-3 §0 事前準備の実行可能性 |
| **L-13b** | **【裁定 2026-08-15・ユーザー】(a) の owner 'system' 特例を採用**（visibility ベースは却下 — visibility は publish 前提条件の語彙であり、一般ユーザーの public まで開放してしまうため）。**〔実装 2026-08-15・DL-1/DL-2〕** 読み取り特例は 3 点のみ: 一覧 `$or`（自分の全件＋system published）・`resolveForCreate`・`resolvePinnedRevision`（`assertRevisionReadableBy`）。findOne と全 mutation は `assertOwner` を維持（緩和変異 MQ1/MQ2 で赤を機械確認）。front は「配布」Badge＋編集/削除非表示。検収記録 = review-results/impl-u14/dl-acceptance.md。**残 = seeder `--execute` の実走（運営作業・手順は document/character-sheet-user-guide.md）と実機確認（D-3 に含める）** | D-3 §0（PH-7 前提 = legacy-coc が DB 解決可能であること） |
| **L-13c** | **gameSystemId の実在性は publish 検証で検査されない（実測 2026-08-15・実機事故）**: seed の `'coc'`（BCDice に実在しない ID）が publish 検証・全 suite を素通りし、実機のキャラ作成（rollOnCreate → StaticLoader.dynamicLoad）で初めて 500 になった。**seed は 'Cthulhu' へ修正済み＋「StaticLoader で実ロードできる」spec で機械固定（DLX）**。ただしガードは seed 限定 — **エディタ製テンプレートは今も実在しない ID を publish でき、作成時ロールで初めて落ちる**。publish 検証への実在性検査の追加（同期 API で StaticLoader の ID 一覧照会が要る・エラー語彙の設計を伴う）は**ユーザー裁定枠**。**〔訂正 2026-08-15・DL-5 実測〕** DLX 時に「試しロール = DiceBot・作成時 = テンプレート方言の非対称が現存」と書いたが**誤り**。server の dice-preview endpoint がテンプレートを解決しないのは事実だが、**front は 2 つの試しロール経路の両方で template.gameSystemId を実際に渡している**（TemplateEditorV3 の 試しロール・TemplatePreviewV3）ため、設定済みなら方言は一致する。**真の非対称は Discord のシート内ロール**（discord/services/dice/dice-roll-logic.service.ts:95-97）で、こちらは gameSystemId を渡さず常に DiceBot 既定。同じキャラの HP 初期化（テンプレート方言）と後続ロール（DiceBot）で方言が割れる。**さらに front が渡すことの機械固定が無い**（preview spec は body 未検証・editor spec は gameSystemId 0 件）ため、front が渡すのをやめても全 suite が緑のまま = DL-5 で pin を追加して塞ぐ | エディタ製テンプレートの作成時ロール信頼性・Discord シート内ロールの方言（別件） |
| **L-13d** | **lookup の miss は per-field 封じ込めなしで evaluateTemplate 全体を throw で落とす（実測 2026-08-15・実機事故 2 件目）**: legacy-coc の damage_bonus 表が 2〜204 しかカバーせず、STR/SIZ 両方未入力（missing = 0 評価）で `lookup did not match` がシート評価全面を停止した。**seed 側は外周を番兵値（-999999/999999）へ拡張し「空 values でも throw しない」spec で機械固定（DLY）。ただしカバーは ±999999 に限る — 入力は有限性のみ要求（value-input）のため、合計が番兵域を超える手入力では miss→クラッシュが再発しうる（大粒度 #24 blocking 指摘・2026-08-15）。本修正 = LookupRow への開放端（{max} / {min} 片側省略）導入で、レビュー実測では同時保持 5→2・ホップ 4→1・番兵 2→0 に純減する。seed 構造変更 = ユーザー DB 再投入を伴うため、実施時期はユーザー裁定（下記 (1) と同時に）**。未解決の一般問題 = (1) engine の lookup miss を per-field 退化（SM-14 の 4 状態語彙）へ封じ込めるか（LookupRow の range 形は min/max 必須で開放端が書けない型制約も含む） (2) publish 検証は lookup 表の全域カバーを要求しないため、**エディタ製テンプレートは今も穴あき表を publish でき、シート表示全体が落ちる**。いずれも**ユーザー裁定枠** | エディタ製テンプレートのシート表示信頼性・legacy キャラクター（backfill pin 経由で値未マッピングのまま評価される面）の表示 |
