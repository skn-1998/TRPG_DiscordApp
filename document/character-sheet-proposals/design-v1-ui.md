...
- **資源上限は「不変条件＋実測由来の集約上限 1 個」**（H-18 改・round1 #12・round2 #3・cogload #5・
  **cross-cut round2 #1 で再修正**）:
  **不変条件（v1 必須）**: 「**publish を通ったテンプレートは runtime step 予算
  （テンプレート全体共有・既定 10,000）内で必ず評価完走する**」。1 式あたり AST 256 の既存上限は不変。
  **集約上限は延期しない**（cogload #5 の「反例が実測できたときのみ導入」条件は**既に成立している**）:
  ...
  最悪コストの算出には **list 行の反復評価**を含める。`PublishValidationOptions.evaluationStepLimit` を
  実際の判定へ接続し（現在は宣言のみ）、**上記 1,024×11 反例を publish/materialize の回帰テストに固定**する。
