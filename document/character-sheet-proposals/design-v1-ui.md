...
- **資源上限は「不変条件＋実測由来の集約上限 1 個」**（H-18 改・round1 #12・round2 #3・cogload #5・
  ...
  **list 行数上限（D-R3 前半 = 案 (a)）**: list 行は **LIST_ROW_LIMIT（既定 512・仮置き）** で
  保存/評価境界において cap し、最悪コスト算出の行反復分は「行上限 × 行内式コスト」で数える。
  実装は表上限（DEFAULT_TABLE_ROW_LIMIT）と同形 = 既定値の単一定数＋options で差し替え可
  （値一致は偶然・定数統合はしない）。仮置き値の変更手順: 上げるときは公開済みテンプレートの
  見積もり再検算が要る・下げるときは既存データの移行裁定が要る（2026-08-12 記録）。
