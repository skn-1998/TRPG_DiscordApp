...
- **trpg-domain-\* スキル = ドメイン別設計ガイド**（`.claude/skills/trpg-domain-{auth,user,character,dice-roll,discord}/`）：
  各ドメインの責務・公開API・やらないこと・既知の落とし穴を定義。対象ドメインが確定したら Phase 1 で該当スキルを読み、
  その**「やらないこと」「既知の落とし穴」を実装委譲プロンプトにそのまま注入**する（サブエージェント/Codex は
  プロジェクト固有の罠——S-1 projection・スレッド保存キー・イベント RPC 禁止等——を知らないため）。
...
- **危険な変更を事前に特定する**：その変更が外部（フロント `trpg-next-app`、DB の既存データ、他モジュール）に
...
固定、④コミット禁止、⑤**対象ドメインの `trpg-domain-*` スキルの「やらないこと」「既知の落とし穴」を
「やってはいけないこと」節へ転記**する。これで成果が均質になり、裏取りしやすくなる。
...
- フロント（trpg-next-app）の改修が必要な変更を、影響確認なしに進めない。
